...
- 生涯题须遵循下文“【生涯题（学生端 5 题）】”的覆盖要求，题干自然表述，避免具体地区政策。
...
`
	userPrompt := fmt.Sprintf(
		"请以 json 对象返回（小写 json），仅输出合法 json：\n"+
			...
			"请基于上述学生信息，生成贴近其校园/选科场景的个性化题干，并严格遵循以下要求：\n"+
			"- **必须**严格遵循 systemPrompt 中所有的数量、结构和维度覆盖要求。\n"+
			"- 个性化题干需遵循“场景多样性”原则，严禁与性别特征关联。\n"+
			"- 题目表述中立，无引导性，严格使用简体中文，JSON 格式合法。",
