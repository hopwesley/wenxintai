...
Page({
  ...
  async loadOrGenerate(publicId: string) {
    ...
    try {
      const res = await get<{ report?: any; need_pay?: boolean }>(
        API_GENERATE_REPORT,
      )
      if (res.report) {
  ...
  startReportStream(publicId: string) {
    ...
    const socket = connectWebSocket(buildReportWsUrl(publicId), {
      onData: (payload) => this.pushLog(String(payload)),
  ...
  handleReportReady(payload: any) {
    ...
    // 这里安全地计算维度字符串
    let dimensionText = ''
    const dims = reportData && (reportData as any).dimensions
    if (Array.isArray(dims)) {
      dimensionText = dims.map((d: any) => String(d)).join('、')
  ...
  async preparePay() {
    ...
    try {
      const prepare = await post<{ order_token: string }>(
        API_PREPARE_PAY,
  ...
  async pollPayStatus(orderToken: string) {
    try {
      const status = await get<{ paid: boolean }>(
        `${API_PAY_STATUS}?order_token=${orderToken}`,
      )
      if (status.paid) {
        await this.confirmReport()
      }
