...
// ========================= 生成问卷（HTTP 版本） =========================
// callAPIForModule 封装单次模块调用并返回 JSON 结果。
func callAPIForModule(module string, mode Mode, apiKey, gender, grade, hobby string) (json.RawMessage, error) {
	...
	if raw == "" {
		return nil, fmt.Errorf("模型返回空内容 for %s", module)
	}

	// --- 最小 JSON 校验（与原逻辑一致） ---
	var tmp any
	if err := json.Unmarshal([]byte(raw), &tmp); err != nil {
		return nil, fmt.Errorf("%s 返回内容非合法 JSON: %w", module, err)
	}
...
func GenerateQuestions(mode Mode, apiKey, gender, grade, hobby string) (*QuestionsResult, error) {
	...
	res := &QuestionsResult{
		...
		Grade:   grade,
		Hobby:   hobby,
		Modules: make(map[string]json.RawMessage, len(modules)),
	}

	for _, module := range modules {
		payload, err := callAPIForModule(module, mode, apiKey, gender, grade, hobby)
		if err != nil {
			return nil, err
		}
		res.Modules[module] = payload
	}
