...
5) 总结建议（**对核心建议及其未来职业衔接进行深化阐述**）
   - **输出 ≤3 条“核心建议”**，每条采用“三段式”：
     1. **行动**（1 句，面向 4–8 周内可执行的关键动作）；
     2. **依据**（1 句，点名引用本报告中的学科/RIASEC/OCEAN 或 mode_support 证据，如“b5_C 高 + 理科向均值 3.67 支持××”）；
     3. **职业衔接**（1–2 句，说明该行动与**类别级别**专业/职业路径的对应关系：高中阶段可参与的**竞赛/项目/实验/志愿/社团**→ 本科阶段的**方向选择**→ 初期职业场景；给出 1 个**可执行连接点**，如“完成××比赛初赛/进入××实验室助理/完成 GIS 入门实践 1 次”）。
   - 以上 3 条“核心建议”可覆盖“学科取舍 / 信息搜集 / 角色认知 / 长期规划 / 兴趣演变”中的重点，但**总数不超过 3 条**；每条建议在行动末尾加一个**时间锚点**（如“第 4 周检查点：提交对比表/完成 2 次体验/达成周测目标”）。
...
4) “总结建议”严格输出 **≤3 条核心建议**，并且**每条均包含“行动/依据/职业衔接”三要素**且带时间锚点；
5) 字数 700–1200；
6) 第 1 部分末尾单行输出“效度：……”；
7) 文末输出“数据来源：request_id / meta.generated_at”（无需展示计算过程）；
8) 全文避免使用“显著”字样，统一改为“明显/更高/更低/更偏向”等中性表述；
9) 如存在 *_ordered 视图，引用顺序以 *_ordered 为准。
