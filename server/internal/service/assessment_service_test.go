...
func TestSubmitFlowGeneratesReport(t *testing.T) {
	repo := newMemoryRepo()
	broker := stream.NewBroker(32, time.Second)
	defer broker.Stop()
	svc := NewSvc(repo, broker)
	...
	svc.interpretReport = func(ctx context.Context, params json.RawMessage, onToken ai.StreamCallback) (json.RawMessage, *string, error) {
		_ = onToken("chunk")
		summary := "ok"
	...
	next, submitResult, err := svc.SubmitAnswersAndAdvance(ctx, qsetID, answerPayload)
	if err != nil {
	...
	nextAgain, submitResultAgain, err := svc.SubmitAnswersAndAdvance(ctx, qsetID, answerPayload)
	if err != nil {
		t.Fatalf("repeat SubmitAnswersAndAdvance stage1 error: %v", err)
	}
	if submitResultAgain != nil {
	...
	nextStage, finalResult, err := svc.SubmitAnswersAndAdvance(ctx, stage2ID, s2Payload)
	if err != nil {
	...
	if finalResult == nil || !finalResult.GenerationStarted {
		t.Fatalf("expected generation to start, got %#v", finalResult)
	}
	...
	for {
		...
		if time.Now().After(deadline) {
			t.Fatalf("GetReport timeout: %v", err)
		}
		time.Sleep(20 * time.Millisecond)
	}
	// Re-submit stage2 after report ready to ensure idempotent report id.
	_, repeatedResult, err := svc.SubmitAnswersAndAdvance(ctx, stage2ID, s2Payload)
	if err != nil {
		t.Fatalf("repeat SubmitAnswersAndAdvance stage2 error: %v", err)
	}
	if repeatedResult == nil || repeatedResult.ReportID == nil || *repeatedResult.ReportID != report.ID {
		t.Fatalf("expected same report id, got %#v", repeatedResult)
	}
...
func (m *memoryRepo) GetInviteForUpdate(ctx context.Context, code string) (*store.Invite, error) {
	return nil, store.ErrNotFound
}

func (m *memoryRepo) UpdateInviteReservation(ctx context.Context, code string, reservedBy *string, reservedUntil *time.Time) error {
	return nil
}

func (m *memoryRepo) RedeemInviteBySession(ctx context.Context, sessionID, redeemedBy string) (bool, error) {
	return false, nil
}

func (m *memoryRepo) RedeemInviteByCode(ctx context.Context, code, redeemedBy string) (bool, error) {
	return true, nil
}
