...
Page({
  ...
  restoreCache(productId: string) {
    ...
    if (cache) {
      this.setData({
        form: { ...this.data.form, ...cache },
      })
    }
  },
  onRegionChange(event: WechatMiniprogram.RegionPickerChange) {
    const [province, city] = event.detail.value
    this.setData({
  ...
  onInterestInput(event: WechatMiniprogram.Input) {
    this.setData({ 'form.interests': event.detail.value })
  },
  ...
  async submitForm() {
    ...
    try {
      const res = await post<{ next_route?: string; public_id?: string }>(API_BASIC_INFO, {
        ...this.data.form,
      ...
      this.setData({ status: err?.message || '提交失败，请稍后再试' })
    } finally {
